Ext.define('Bling.controller.wms.OutwardProcessingEditor', {
    ...
    init: function() {
        var me = this;
        this.control({
            'outward-processing-editor #addArticlesBtn': {
                click: this.openAddArticlesWin
            },
            'outward-processing-editor tally-article-grid': {
            ...
            'outward-processing-editor #deleteBtn': {
                click: this.deleteArticle
            },
            'outward-processing-editor #executeBtn': {
                click: this.execute
            }
        })
    },
    ...
    openAddArticlesWin: function(cmp) {
        ...
        win.on('add-articles', function(array) {
            Ext.Ajax.request({
                url: '/eis/wms/outward-processings/' + orderId + '/articles',
                method:'POST',
                jsonData:array,
    ...
    getEditor: function(cmp) {
        return cmp.up('outward-processing-editor');
    },
    execute: function(btn) {
        var me = this;
        var id = this.getOrderId(btn);
        var executeUrl = '/eis/wms/outward-processings/' + id + '/execute';
