Ext.define('Bling.controller.wms.SellOrderMgmtGrid', {
    extend: 'Ext.app.Controller',
    requires: ['Bling.view.wms.SellOrderEditor'],
    views: ['wms.SellOrderEditor'],
    refs: [
        {
            selector: 'sell-order-mgmt-grid',
            ref: 'mainGrid'
        },
        {
    ...
    init: function() {
        var me = this;
        this.control({
            'sell-order-mgmt-grid #createSellBtn': {
                click: this.createOrder
            },
            'sell-order-mgmt-grid grid': {
    ...
    initOrderEditor: function(data) {
       ...
       //var panel = this.getView('wms.ReceivingOrderEditor').create({articleStore: store});
        var panel = this.getView('wms.SellOrderEditor').create({articalStore:store});
        store.load();
