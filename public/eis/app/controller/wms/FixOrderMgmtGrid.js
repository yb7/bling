Ext.define('Bling.controller.wms.FixOrderMgmtGrid', {
    extend: 'Ext.app.Controller',
    requires: ['Bling.view.wms.FixOrderEditor'],
    views: ['wms.FixOrderEditor'],
    refs: [
        {
            selector: 'fix-order-mgmt-grid',
            ref: 'mainGrid'
        },
        {
    ...
    init: function() {
        var me = this;
        this.control({
            'fix-order-mgmt-grid #createSellBtn': {
                click: this.createOrder
            },
            'fix-order-mgmt-grid grid': {
    ...
    initOrderEditor: function(data) {
       ...
       //var panel = this.getView('wms.ReceivingOrderEditor').create({articleStore: store});
        var panel = this.getView('wms.FixOrderEditor').create({articalStore:store});
        store.load();
