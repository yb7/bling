Ext.define('Bling.controller.wms.CustomOrderMgmtGrid', {
    extend: 'Ext.app.Controller',
    requires: ['Bling.view.wms.CustomOrderEditor'],
    views: ['wms.CustomOrderEditor'],
    refs: [
        {
            selector: 'custom-order-mgmt-grid',
            ref: 'mainGrid'
        },
        {
    ...
    init: function() {
        var me = this;
        this.control({
            'custom-order-mgmt-grid #createSellBtn': {
                click: this.createOrder
            },
            'custom-order-mgmt-grid grid': {
    ...
    initOrderEditor: function(data) {
       ...
       //var panel = this.getView('wms.ReceivingOrderEditor').create({articleStore: store});
        var panel = this.getView('wms.CustomOrderEditor').create({articalStore:store});
        store.load();
