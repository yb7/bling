Ext.define('Bling.controller.wms.CounterOrderMgmtGrid', {
    extend: 'Ext.app.Controller',
    requires: ['Bling.view.wms.PreOrderEditor'],
    views: ['wms.CounterOrderEditor'],
    refs: [
        {
            selector: 'counter-order-mgmt-grid',
            ref: 'mainGrid'
        },
        {
    ...
    init: function() {
        var me = this;
        this.control({
            'counter-order-mgmt-grid #createSellBtn': {
                click: this.createOrder
            },
            'counter-order-mgmt-grid grid': {
    ...
    initOrderEditor: function(data) {
       ...
       //var panel = this.getView('wms.ReceivingOrderEditor').create({articleStore: store});
        var panel = this.getView('wms.CounterOrderEditor').create({articalStore:store});
        store.load();
