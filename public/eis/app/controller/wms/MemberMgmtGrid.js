Ext.define('Bling.controller.wms.MemberMgmtGrid', {
    extend: 'Ext.app.Controller',
    requires: ['Bling.view.wms.MemberEditor'],
    views: ['wms.MemberEditor'],
    refs: [
        {
            selector: 'member-mgmt-grid',
            ref: 'mainGrid'
        },
        {
    ...
    init: function() {
        var me = this;
        this.control({
            'member-mgmt-grid #createSellBtn': {
                click: this.createOrder
            },
            'member-mgmt-grid grid': {
    ...
    initOrderEditor: function(data) {
       ...
       //var panel = this.getView('wms.ReceivingOrderEditor').create({articleStore: store});
        var panel = this.getView('wms.MemberEditor').create({articalStore:store});
        store.load();
