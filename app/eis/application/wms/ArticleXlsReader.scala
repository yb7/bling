...
import com.wyb7.waffle.commons.xls.{ErrorMessage, RowMapper, ReadXlsToMap}
...
trait ArticleXlsReader {
    def read(workbook: HSSFWorkbook): Either[ErrorMessage, List[Article]]
}
...
class ArticleXlsReaderImpl @Autowired()(companyRepository: CompanyRepository) extends ArticleXlsReader {
    ...
    implicit private def hssfCellToCompany(cell: HSSFCell): Company = {
        val companyName: String = cell
        companyName.notEmptyOp.flatMap(companyRepository.findByName(_)).getOrElse(null)
    }

    def read(workbook: HSSFWorkbook): Either[ErrorMessage, List[Article]] =  {
        val sheet = workbook.getSheetAt(0)
        val reader = new ReadXlsToMap(sheet, Set.empty[String])
        reader.read(new RowMapper[Article] {
            def map(values: Map[String, HSSFCell], errorMessage: ErrorMessage, rowCount: Int) = {
                val article = new Article
                article.code = values("货品编码")
                article.vendor1 = values("供货商1")
                article.vendor2 = values("供货商2")
                article.vendor3 = values("供货商3")
                ...
                article.sideStoneName = values.get("辅石名称")
                article.sideStoneCount = values.get("辅石数量")
                article.sideStoneWeight = values.get("辅石重量")
                article.sideStoneCode = values.get("辅石石号")
